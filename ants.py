...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if self.exit != None: 
            exit.entrance = self #the exit's entrance will be this new object
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place != beehive:
            if self.min_range <= entrance_transitions <= self.max_range:
                if rANTdom_else_none(current_place.bees): #if got bee(s)
                    return rANTdom_else_none(current_place.bees) #return a random bee
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        curr_place_bees = self.place.bees[:]
        Ant.reduce_armor(self, amount)
        damage_received = initial_armor - self.armor
        for bee in curr_place_bees:
            if self.place == None: #when insect dies, the place becomes None
                Insect.reduce_armor(bee, self.damage + damage_received)
            else:
                Insect.reduce_armor(bee, damage_received)
...
# END Problem EC
    ...
    implemented = True   # Change to True to view in the GUI
    # END Problem EC
    #queen_ant cannot be removed, override Ant.remove method

    def __init__(self, armor=1):
        # BEGIN Problem EC
        ScubaThrower.__init__(self, armor)
        self.true_queen = QueenAnt.true_queen + 1
        QueenAnt.true_queen += 1 #all the queens die
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.true_queen > 1:
            ...
            while curr_exit != None:
                if curr_exit.ant:
                    if curr_exit.ant.name != 'doubled': ##change name of ant, not the queen
                        curr_exit.ant.damage = 2 * curr_exit.ant.damage
                        curr_exit.ant.name = 'doubled'
                curr_exit = curr_exit.exit

        #check if there are ants
        #if got ants, check makesure self.name != 'doubled'
        #double damage and make self.name = 'doubled'
        #else, just move on to the next exit
        # END Problem EC
    ...
    def reduce_armor(self, amount):
        ...
        if self.true_queen == 1 and self.armor <= 0:
            bees_win()
        # END Problem EC
